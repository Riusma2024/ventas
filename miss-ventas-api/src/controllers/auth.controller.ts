    ...
    try {
        ...
        res.json({ 
            ...
            usuario: { 
                ...
                rol: usuario.rol, 
                tenant_id: usuario.tenant_id, 
                negocio_slug: usuario.negocio_slug 
            } 
        });
    } catch (error: any) { 
...
export const registrarVendedor = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (codigo_cupon) {
            ...
            if (cupon.length > 0) { 
                diasPrueba = cupon[0].dias_regalo; 
                await db.query('UPDATE cupones SET usos_actuales = usos_actuales + 1 WHERE id = ?', [cupon[0].id]); 
            }
        }
        
        const fechaExpira = new Date(); 
        fechaExpira.setDate(fechaExpira.getDate() + diasPrueba);
        
        const hash = await bcrypt.hash(password, 10);
        const [result] = await db.query<ResultSetHeader>(
            'INSERT INTO usuarios (nombre, email, password_hash, rol, negocio_nombre, negocio_slug, sub_expira_el, sub_status) VALUES (?, ?, ?, "vendedor", ?, ?, ?, "trial")', 
            [nombre, email, hash, negocio_nombre, slug, fechaExpira]
        );
        
        const newId = result.insertId; 
        await db.query('UPDATE usuarios SET tenant_id = ? WHERE id = ?', [newId, newId]);
...
export const solicitarRecuperacion = async (req: Request, res: Response): Promise<void> => {
    ...
    } catch (error) { res.status(500).json({ error: 'Error en recuperación' }); }
};
...
export const restablecerPassword = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const [user]: any = await db.query(
            'SELECT id FROM usuarios WHERE email = ? AND reset_token = ? AND reset_token_expires > NOW()', 
            [email, token]
        );
